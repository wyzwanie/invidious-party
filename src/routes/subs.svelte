...
// channelID -> gun -> if in gun check update or refresh?
    ...
    const job = async (task, next) => {
        ...
        try {
            ...
            // if(SUBfeed.findIndex(x => x.authorId === channelID) === -1) {
                ...
                // if(fetchChannel.success) {
                //     if(SUBfeed.findIndex(x => x === fetchChannel.success) < 0) SUBfeed = [...SUBfeed, fetchChannel.success]
                // }
    ...
    // let wyslano = count(oryginalTasks)
    // let udane = count(successTasks)
    // let failed = count(failedTasks)

    const count = queue => {
        let wynik = {}
        queue.forEach(task => {
            const { instance } = task
            if(wynik[instance]) wynik[instance]++
            else wynik[instance] = 1
        })
        return wynik
    }
    $: console.log('wyslano', count(oryginalTasks))
    $: console.log('udane', count(successTasks))
    $: console.log('failed', count(failedTasks))
    ...
    const SUBqueue = Queue.channels(concurrency)
        ...
        .success(task => {
            ...
            tasks = tasks.filter(x => x.channelID !== task.channelID)
            reQueue = reQueue.filter(x => x.channelID !== task.channelID)
            workingInstances = workingInstances.includes(task.instance) ? workingInstances : [...workingInstances, task.instance]
            successTasks = [...successTasks, task]
        })
        .failure((err, task) => {
            failedTasks = [...failedTasks, task]
            const empty_response = typeof err.reason === 'object' && Object.keys(err.reason).length === 0
            const failed_fetch = err.reason === 'Failed to fetch' || err.reason === 'Read timed out' || err.message === 'Process timed out'

            if(err.init || empty_response || failed_fetch) {
    ...
    const feCh = async (instance, channelID, signal) => {
        if(SUBfeed.findIndex(x => x.authorId === channelID) > 0) return { failed: channelID, reason: 'duplicate' }
        // if(instance === null) return { init: 'success' }
        // if(workingInstances.length > 3) instance = workingInstances[Math.floor(Math.random()*workingInstances.length)]
        inProgress++
        requestsMade++

        // if(count(failedTasks)[instance]) {
        //     const count = count(successTasks)
        //     const len = Object.keys(count).length
        //     const random = Math.floor(Math.random()*len)
        //     const index = Object.keys(count)[random]
            
        //     instance = count(successTasks)[index]
        // }
    ...
    const refresh = (refreshedAt, instances, SUBs) => {
        ...
        if(first_time || need_refresh) {
            ...
            tasks = SUBs.map((channelID, index) => {
                ...
                return {
                    instance: ynstancez[index] ? ynstancez[index].instance : ynstancez[Math.floor(Math.random()*ynstancez.length)].instance,
                    channelID,
                    interval: 500,
                    control,
            ...
            oryginalTasks = tasks
            start = oryginalTasks.length
            concurrency = Math.floor((Math.floor(tasks.length / (Math.log(tasks.length)*1.618)))*1.618)
            SUBfeed.length = 0

            tasks.forEach(task => SUBqueue.add(task))
        }
    }
    // $: console.log(workingInstances)
    const startYmport = () => {
    ...
    $: if($instances && $instances.length) {
        ...
        setInterval(() => refresh($SUBsRefreshedAt, $instances, $SUBs), 90 * 60 * 1000)
    }
    // done?
$: console.log($SUBs)
...
$: if(doneCount === start) {
    console.log(start, doneCount, SUBfeed.length, reQueue.length, tasks.length)
    doneCount = 0
    reQueue.forEach(task => {
        if(tasksDone.findIndex(x => x.channelID === task.channelID) < 0) {
            const count = count(successTasks)
            const len = Object.keys(count).length
            const random = Math.floor(Math.random()*len)
            const index = Object.keys(count)[random]
...
<hr>
SUBs: {oryginalTasks.length}<br>
tasks: {tasks.length}<br>
reQueue: {reQueue.length}<br>
tasksDone: {tasksDone.length}<br>
doneCount: {doneCount} | {doneCount === 333}<br>
SUBfeed: {SUBfeed.length}<br>
<hr>
<div class="test">
    reQueue<hr>
    <!-- {#key reQueue} -->
    {#each Object.keys(count(reQueue)) as instance}
        {instance} : {count(reQueue)[instance]}<br>
    {/each}
    <!-- {/key} -->
    <br>

    tasksDone<hr>
    {#each Object.keys(count(tasksDone)) as instance}
    {instance} : {count(tasksDone)[instance]}<br>
    {/each}
    <br>

    wyslano<hr>
    {#each Object.keys(count(oryginalTasks)) as instance}
        {instance} : {count(oryginalTasks)[instance]}<br>
    {/each}
    <br>
    udane<hr>
    {#each Object.keys(count(successTasks)) as instance}
        {instance} : {count(successTasks)[instance]}<br>
    {/each}
    <br>
    failed<hr>
    {#each Object.keys(count(failedTasks)) as instance}
        {instance} : {count(failedTasks)[instance]}<br>
    {/each}
    <br>
</div>
