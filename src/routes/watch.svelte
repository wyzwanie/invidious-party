<script>
	...
	import { consent, instances } from '$lib/Stores/localStore'
    ...
    import VideosRelated from '$lib/Components/VideosRelated.svelte'
    ...
    const runFetcher = (instance, videoID, validVideoID) => {
        if(!instance || fetcher.running || !validVideoID) return
...
{#if validVideoID}
    <div class="wrapper">
        ...
        {:else}
            {#if !error}
                {#if videoAPI && videoAPI.error && !videoAPI.authorId}
                    <AsyncError error={videoAPI.error} />
                {:else}
                    <div class="top">
                        <div class="video">
                            {#if $chosen}
                                <Video chosen={$chosen} {videoAPI} on:height={e => height = e.detail}/>
                            {/if}
                        </div>
                        {#if $consent === 'party'}
                            <div class="chat">
                                <Chat roomID={videoID} {height} />
                            </div>
                        {/if}
                        {#if $consent === 'solo'}
                            <div class="related">
                                ...
                                <VideosRelated chosen={$chosen} videos={videoAPI.recommendedVideos} {height} />
                            </div>
                        {/if}
                    </div>
