<script>
    ...
    const getGudInstance = (instancesFail) => {
        ...
        Object.keys(instancesFail).forEach(key => {
            ...
            if(total > 0) {
                if(instancesFail[key][0] < 3 && instancesFail[key][1] > 0 && key) wynik.push(key)
            } else wynik.push(key)
        })
